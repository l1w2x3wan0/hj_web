...
abstract class BaseAction extends Action{
	public function _initialize(){
			...
			if(!isset($_SESSION['js_power']) && empty($_SESSION['js_power'])){
				$this->error('您没有该权限，请联系管理员',$link);
			}else{
				$js_power = json_decode($_SESSION['js_power'], true);
				...
				if ($_SESSION['js_flag']=="1"){
					...
					$count1 = $shenhe->where("cate in (101,102,103) and flag=0")->count('id'); 
					if ($count1 > 0) $shenhe_msg .= empty($shenhe_msg) ? "商品管理" : "，商品管理";
					$count5 = $shenhe->where("cate in (121,122,123) and flag=0")->count('id'); 
					if ($count5 > 0) $shenhe_msg .= empty($shenhe_msg) ? "奖券商城" : "，奖券商城";
					$count2 = $shenhe->where("cate=104 and flag=0")->count('id'); 
					if ($count2 > 0) $shenhe_msg .= empty($shenhe_msg) ? "公告管理" : "，公告管理";
					$count3 = $shenhe->where("cate<51 and flag=0")->count('id'); 
					if ($count3 > 0) $shenhe_msg .= empty($shenhe_msg) ? "基本配置" : "，基本配置";
					$count4 = $shenhe->where("cate>50 and cate<100 and flag=0")->count('id'); 
					if ($count4 > 0) $shenhe_msg .= empty($shenhe_msg) ? "小游戏配置" : "，小游戏配置";
					if (!empty($shenhe_msg)) $shenhe_msg .= "有需要审核的内容，请尽快审核!";
					*/
					$this->assign('shenhe_msg',$shenhe_msg);
