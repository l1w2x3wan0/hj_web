...
class UserAction extends BaseAction {
	...
	public function waitshang(){
		...
		$count = $rowlist->where("cate in (101,102) $sql0")->count('id');
		$Page       = new Page($count,PAGE_SHOW);//实例化分页类传入总记录数和每页显示的记录数		
		$pageshow       = $Page->show();// 分页显示输出
		
		$list = $rowlist->where("cate in (101,102) $sql0")->order('flag,id DESC')->limit($Page->firstRow.','.$Page->listRows)->select();
		//dump($rowlist->_sql());
	...
	public function waitshang_show(){
		...
		$this->assign('By_tpl',$this->By_tpl);
		$lib_display = $this->By_tpl.":waitshang_show";
		$this->display($lib_display);
	}
