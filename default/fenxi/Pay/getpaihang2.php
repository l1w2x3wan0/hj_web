...
$sql = "select * from user_info where $sql1  order by charm DESC LIMIT $shownum";
//echo $sql;
$res = mysql_query($sql, $conn2);
$sql1 .= " and user_id not in (";
$user_service = "";
while($row = mysql_fetch_array($res)){
	//echo empty($row['nickname']) ? $row['nick_name'] : $row['nickname']; echo '<br>';
	...
	$show[$key]['tx'] = !empty($row['head_picture']) ? $img : "0";
	$show[$key]['gender'] = $row['sex'];
	$show[$key]['nickname'] = $nick;
	$show[$key]['place'] = $key+1;
	$show[$key]['total'] = (int)$row['charm'];
	$show[$key]['type'] = 2;
	...
	$user_service .= empty($user_service) ? $row['user_id'] : "_".$row['user_id'];
}
$sql1 .= ")";
//print_r($show);	
	$flag = 6;
