...
$show = array();
$gold9 = array();
$sql1 = " !((ud.user_id>=".ROBERT1_BEGIN." and ud.user_id<".ROBERT1_END.") or (ud.user_id>=".ROBERT2_BEGIN." and ud.user_id<=".ROBERT2_END."))";
$key=0;
...
$can3 = $row['tongji'];
if (!empty($can2)){
	$sql1 .= " and ud.user_id not in ($can3)";
}

$shownum = RANKING_ASSETS;
$shownum = !empty($shownum) ? $shownum : 50;
$sql = "select ud.today_diamond,ud.user_id as user_id2,u.* FROM user_temple_data_record ud LEFT JOIN user_info u ON ud.user_id=u.user_id where $sql1 and ud.today_diamond>0 order by ud.today_diamond DESC LIMIT $shownum";
//echo $sql;
$res = mysql_query($sql, $conn2);
...
while($row = mysql_fetch_array($res)){
	...
	$show[$key]['total'] = (int)$row['today_diamond'];
	$show[$key]['type'] = 1;
	$show[$key]['uid'] = $row['user_id2'];
	//$show[$key]['vip_type'] = $row['viplevel'];
	
	$sql0 = "select IF(u.viplevel>=IFNULL(c.viplevel,0),u.viplevel,c.viplevel) as maxlevel from user_info as u left join `log_change_user_vip` as c on u.`user_id` =c.`user_id`  where u.user_id=".$row['user_id2'];
